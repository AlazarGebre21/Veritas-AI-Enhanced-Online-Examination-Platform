...
import { useQuestions, useDeleteQuestion } from "../hooks/useStaffQuestions.js";
...
export default function StaffSubjectQuestionsPage() {
  const { subjectName } = useParams();
  const navigate = useNavigate();

  // Fetch all questions to filter by subject locally
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        onRowClick={(row) => navigate(ROUTES.STAFF_QUESTION_DETAIL.replace(":id", row.id))}
      />
    </div>
  );
}
